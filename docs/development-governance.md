...
- When the right-side branch details or handoff summary lists sub-agents, show more than the nickname. Include the assigned role, model/version when known, thinking/reasoning effort, and current state. Use a compact format such as `Jason — Reviewer / Evaluator — GPT-5.5 Thinking / Medium — closed after timeout`.
