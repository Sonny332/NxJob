...
- On Windows, agents and workers must run Python tests through `scripts/run_pytest.ps1` or `scripts/test-local-service.ps1` instead of raw `python -m pytest`. This avoids the known Python 3.14 / pytest temporary-directory ACL failure on this host and keeps test runtime artifacts under the Windows temp folder instead of the repository.
