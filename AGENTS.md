...
- On Windows, agents and workers must run Python tests through `scripts/run_pytest.ps1` or `scripts/test-local-service.ps1` instead of raw `python -m pytest`. The harness disables pytest cache by default and applies the already-approved process-local ACL compatibility only inside the test process; `test-local-service.ps1` also disables Python bytecode generation.
- If pytest hits `PermissionError`, `WinError 5`, `Access is denied`, `pytest-of-<username>`, or `pytest-cache-files-*`, stop repeating the same sandboxed command. Retry only the exact project wrapper command outside the Codex filesystem sandbox, using the current ordinary Windows user; sandbox bypass is not administrator elevation.
- Never use `RunAs`, an administrator terminal, UAC elevation, or a system account for pytest. Never recursively grant `FullControl` to Python, the repository, the user TEMP directory, or pytest temp directories. Request the narrowest sandbox exception for the specific wrapper command, not broad Python or PowerShell execution.
- Do not assume changing `TEMP` or `--basetemp` alone fixes the ACL conflict. When cleaning failed pytest directories, verify the resolved absolute path and delete only the specific pytest/NxJob test runtime directory outside the sandbox; never broaden parent-directory ACLs.
- The existing `scripts/run_pytest.py` compatibility is project-scoped and test-only. Do not modify global Python, pytest in `site-packages`, system ACLs, or production code. Replacing or extending this compatibility requires explicit user approval.
- Test reports for Windows pytest must state the exact wrapper command, whether it ran outside the Codex sandbox, whether it remained non-admin, pass/fail counts, and whether temporary directories were created or cleaned.
